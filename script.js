...
/**
 * Enquiry Popup Form Script
 * Non-intrusive popup with smart triggers and session management
 */
...
(function () {
  ...
  function wasRecentlyDismissed() {
    ...
    return daysSinceDismissed < CONFIG.dismissDuration;
  }
  ...
  function initTriggers() {
    ...
    // Time-based trigger
    setTimeout(function () {
      if (!hasTriggered && !wasRecentlyDismissed()) {
        hasTriggered = true;
        showPopup();
      }
    }, CONFIG.triggerDelay);
    ...
    let scrollHandler = function () {
      ...
      if (scrollPercent >= CONFIG.scrollTrigger && !wasRecentlyDismissed()) {
        hasTriggered = true;
        showPopup();
  ...
  function initEvents() {
    ...
    const triggerBtn = document.getElementById('enquiry-trigger-btn');
    if (triggerBtn) {
...
$(function () {
  ...
  // 5. Product Filtering Logic (products.html)
  const checkboxes = document.querySelectorAll('.filter-checkbox');
  const productItems = document.querySelectorAll('.product-item');
  const noProductsMsg = document.getElementById('no-products-msg');

  if (checkboxes.length && productItems.length) {
    function filterProducts() {
      ...
      checkboxes.forEach(cb => {
        if (cb.checked) {
          selectedFilters[cb.dataset.group].push(cb.value);
        }
      });
      ...
      productItems.forEach(product => {
        ...
        const productGrade = product.dataset.grade;
        const productGsm = product.dataset.gsm ? product.dataset.gsm.split(',') : [];

        const typeMatch = selectedFilters.type.length === 0 || selectedFilters.type.includes(productType);
        const gradeMatch = selectedFilters.grade.length === 0 || selectedFilters.grade.includes(productGrade);
        const gsmMatch = selectedFilters.gsm.length === 0 || selectedFilters.gsm.some(g => productGsm.includes(g));

        if (typeMatch && gradeMatch && gsmMatch) {
  ...
  // 6. Smooth Scroll for FAQ (faq.html)
  const faqLinks = document.querySelectorAll('.faq-quick-links a');
  if (faqLinks.length) {
    faqLinks.forEach(anchor => {
      anchor.addEventListener('click', function (e) {
        ...
        if (targetId && targetId.startsWith('#')) {
          ...
          if (target) {
            ...
            faqLinks.forEach(a => a.classList.remove('active'));
  ...
  =============================================*/
  var progressPath = document.querySelector('.advanced-scroll-top path');
  if (progressPath) {
    ...
    jQuery(window).on('scroll', function () {
      if (jQuery(this).scrollTop() > offset) {
        jQuery('.advanced-scroll-top').addClass('active-progress');
      } else {
        jQuery('.advanced-scroll-top').removeClass('active-progress');
      }
    });
    jQuery('.advanced-scroll-top').on('click', function (event) {
      event.preventDefault();
...
// Testimonial slideshow: show one testimonial at a time and auto-advance every 3s

// Custom slideshow removed to avoid conflict with Slick slider (handled by Slick)



/* === New stats counter + modern testimonial slider (self-contained, no external libs) === */
(function () {
  'use strict';

  // --- Counters: robust 2s animated counter with fallback and MutationObserver ---
  try {
    ...
    function animateCard(card) {
      try {
        if (!card) return;
        // If an animation is already running for this card, cancel it
        var token = {};
        card._animToken = token;
        ...
        var startTime = null;
        function step(ts) {
          if (card._animToken !== token) return; // aborted by newer animation
          if (!startTime) startTime = ts;
          var elapsed = ts - startTime;
          var progress = Math.min(elapsed / duration, 1);
          var eased = easeOutQuad(progress);
          var targetRaw = parseFloat(card.getAttribute('data-target'));
          var target = isNaN(targetRaw) ? 0 : targetRaw;
          ...
          // finalise and add suffix
          if (suffix) { valueEl.innerHTML = current + '<span class="suffix">' + suffix + '</span>'; }
          else { valueEl.textContent = current; }
          card.dataset.animated = '1';
        }
        requestAnimationFrame(step);
      } catch (err) { console.error('animateCard error:', err); }
    }

    var statsArea = document.querySelector('.new-statistics-area');
    if (statsArea) {
      var cards = Array.prototype.slice.call(statsArea.querySelectorAll('.stat-card'));

      // trigger function used by both IntersectionObserver and fallback
      function triggerStats() {
        cards.forEach(function (c) { animateCard(c); });
      }

      // Observe attribute changes (data-target/data-suffix) to re-animate if updated
      try {
        var mo = new MutationObserver(function (mutations) {
          mutations.forEach(function (m) {
            if (m.type === 'attributes' && (m.attributeName === 'data-target' || m.attributeName === 'data-suffix')) {
              animateCard(m.target);
            }
          });
        });
        cards.forEach(function (c) { mo.observe(c, { attributes: true, attributeFilter: ['data-target', 'data-suffix'] }); });
      } catch (ignore) { /* MutationObserver unsupported - skip */ }

      if ('IntersectionObserver' in window) {
        var obs = new IntersectionObserver(function (entries, observer) {
          entries.forEach(function (entry) { if (entry.isIntersecting) { triggerStats(); observer.disconnect(); } });
        }, { threshold: 0.45 });
        obs.observe(statsArea);
        // fallback in case observer never fires (some mobile browsers)
        setTimeout(function () {
          if (!cards.some(function (c) { return c.dataset.animated === '1'; })) { triggerStats(); }
        }, 800);
      } else {
        // no observer - trigger immediately
        triggerStats();
      }
    }
  } catch (err) { console.error('Stats init error:', err); }

  // --- Slider: Infinite Continuous Autoplay 5s, arrows, dots, pause on hover, keyboard, touch ---
  try {
    var slider = document.querySelector('.mt-slider');
    if (slider) {
      var track = slider.querySelector('.mt-track');
      var originalSlides = Array.prototype.slice.call(track.querySelectorAll('.mt-slide'));
      var prevBtn = slider.querySelector('.mt-prev');
      var nextBtn = slider.querySelector('.mt-next');
      var dotsContainer = slider.querySelector('.mt-dots');
      var idx = 1; // Start at 1 because we prepended a clone
      ...
      firstClone.classList.add('mt-clone');
      lastClone.classList.add('mt-clone');

      track.appendChild(firstClone);
      track.insertBefore(lastClone, originalSlides[0]);

      // Re-query all slides including clones
      var allSlides = Array.prototype.slice.call(track.querySelectorAll('.mt-slide'));

      // Build dots based on original length
      if (dotsContainer) {
        dotsContainer.innerHTML = '';
        originalSlides.forEach(function (s, i) {
          var btn = document.createElement('button');
          btn.className = 'mt-dot';
      ...
      function updateDots() {
        ...
        var dotIdx = idx - 1;
        if (idx === 0) dotIdx = originalSlides.length - 1;
        if (idx === allSlides.length - 1) dotIdx = 0;

        var ds = dotsContainer.querySelectorAll('.mt-dot');
      ...
      function go(targetIdx, animate) {
        ...
        track.style.transition = animate ? 'transform .75s cubic-bezier(.22, .9, .42, 1)' : 'none';
      ...
      track.addEventListener('transitionend', function () {
        ...
        // If we are on a clone, jump to the real slide without animation
        if (idx === 0) {
          go(allSlides.length - 2, false);
      ...
      slider.addEventListener('keydown', function (e) {
        if (isTransitioning) return;
        if (e.key === 'ArrowLeft') { stop(); go(idx - 1); start(); }
        else if (e.key === 'ArrowRight') { stop(); go(idx + 1); start(); }
      });

      function start() {
        stop();
        timer = setInterval(function () { go(idx + 1); }, AUTO_DELAY);
      }
  ...
  } catch (err) { console.error('Testimonial slider infinite init error:', err); }
})();
